...
def choosecategory():
         ...
         top=Toplevel(gui)
         gui.withdraw()
         #this function keeps the root functioning however hides it from the output screen
         ...
         #Animal category
         category1 = Label(top,text="ANIMAL",font=("Georgia",14),bg='white')
         ...
         #fruit category
         category2 = Label(top,text="FRUIT",font=("Georgia",14),bg='white')
         ...
         #country category
         category3 = Label(top,text="COUNTRY",font=("Georgia",14),bg='white')
...
def hidebutton(let):
     let.destroy()
     #fucntion to hide the last two pop up screen and restart the game after finshing the game
def removescreen(widget):
     widget.withdraw()
     choosecategory()
# popup window for main game
def popup():
     global text_change_label,attempts_label,attempts,my_Label,root,top
     global img0,img1,img2,img3,img4,img5,img6,img7,img8,img9,img10
     root=Toplevel(gui)
     ...
     attempts_label = Label(root, text=" ",font=("Georgia",12),bg='white') #attempts left ch label
     attempts_label.place(x=560,y=120)
     #attempts_label.config(text='ATTEMPTS LEFT: '.join(str(attempts)))
     guess_the_word_label = Label(root, text="GUESS THE WORD!!",
     font=("Georgia",14),bg='white') #guess the word asa lihun yeil
     ...
     z.place(x= 940,y = 442)
     hint = Button(root, text=' Hint ', fg='black', bg='salmon3', height=1,
     width=7,command=lambda:clickedhint(hint))
...
def youwin():
     ...
     replay_btn = Button(chicken,image = replay,command=lambda:removescreen(chicken))
     replay_btn.pack()
     exit2 = PhotoImage(file="exit_button.png")
     exit_label2 = Label(chicken, image = exit2)
     exit_btn2 = Button(chicken, image= exit2,command=lambda:hidebutton(gui))
     exit_btn2.pack()
     chicken.mainloop()
...
def youloose(word):
     rat = Toplevel(gui)
     rat.title('oh no :(')
     rat.geometry('300x230')
     you_lost_label = Label(rat,text = "YOU LOST :(",font=("Georgia",16),bg='white')
     you_lost_label.pack()
     label=Label(rat,text="the word was:",font=("Georgia",12),bg="white")
     label.pack()
     labell=Label(rat,text=word,font=("Georgia",16),bg="white")
     labell.pack()
     replay1 = PhotoImage(file="Replay_button.png")
     replay1_label = Label(rat,image=replay1)
     replay_btn1 = Button(rat,image = replay1,command=lambda:removescreen(rat))
     replay_btn1.pack()
     exit3 = PhotoImage(file="exit_button.png")
     exit_label3 = Label(rat, image = exit3)
     exit_btn3 = Button(rat, image= exit3,command=lambda:hidebutton(gui))
     exit_btn3.pack()
     rat.mainloop()
#Rules window
def rules():
...
# function for quit
exit_btn = Button(gui,image=exit1,command=lambda:hidebutton(gui))
